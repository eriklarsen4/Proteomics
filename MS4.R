...
## Add a column that will be comprised of human gene names
...
MS_DF$`Molecular Weight` = as.numeric(gsub(" kDa","", MS_DF$`Molecular Weight`))
## Coerce the MW column to be numeric to be sure
MS_DF$`Molecular Weight` = as.numeric(MS_DF$`Molecular Weight`)
## Remove the decoy proteins
MS_DF_filter = MS_DF %>% filter(!grepl(MS_DF$`Accession Number`, pattern = "DECOY"))

  ##### Additional revisions; normalizing counts for plotting #####

## Remove proteins under 5 raw counts in TMEM sample
...
## Scale counts by the MWs of their proteins
MS_DF_filter$myc_GFP_1 = as.numeric(MS_DF_filter$myc_GFP_1)/as.numeric(MS_DF_filter$`Molecular Weight`)
MS_DF_filter$myc_GFP_2 = as.numeric(MS_DF_filter$myc_GFP_2)/as.numeric(MS_DF_filter$`Molecular Weight`)
MS_DF_filter$TMEM184B_myc_1 = as.numeric(MS_DF_filter$TMEM184B_myc_1)/as.numeric(MS_DF_filter$`Molecular Weight`)
MS_DF_filter$TMEM184B_myc_2 = as.numeric(MS_DF_filter$TMEM184B_myc_2)/as.numeric(MS_DF_filter$`Molecular Weight`)
...
MS_DF_filter = add_column(MS_DF_filter, TMEM_RATIO = as.numeric(MS_DF_filter$TMEM184B_myc_1) / as.numeric(MS_DF_filter$TMEM184B_myc_2))

## Remove proteins that have more counts in the 2nd TMEM sample
MS_DF_filter = subset(MS_DF_filter, MS_DF_filter$TMEM_RATIO > 1.0)
## Remove Keratin and Tubulin, common contaminants or false positives
MS_DF_filter = MS_DF_filter %>% filter(!grepl(MS_DF_filter$`Identified Proteins (6608)` , pattern = "Keratin.+?$|Tubulin.+?$"))


## Average the counts from both conditions
MS_DF_filter = add_column(MS_DF_filter, GFPavg = (as.numeric(MS_DF_filter$myc_GFP_1) + as.numeric(MS_DF_filter$myc_GFP_2))/2)
MS_DF_filter = add_column(MS_DF_filter, TMEMavg = as.numeric(MS_DF_filter$TMEM184B_myc_1) + as.numeric(MS_DF_filter$TMEM184B_myc_2)/2)

## Add a regularization term to quantify counts that scale by counts and penalize proteins that deviate from the TMEM ratio
MS_DF_filter$scale = ((MS_DF_filter$TMEMavg*MS_DF_filter$TMEM_RATIO)-(MS_DF_filter$TMEMavg*sqrt((MS_DF_filter$TMEM_RATIO - MS_DF_filter$TMEM_RATIO[1])**2)))

## Add a scoring metric that provides a distance metric from TMEM; further distances = lower scores
MS_DF_filter = add_column(MS_DF_filter, score = 1/sqrt((MS_DF_filter$GFPavg[1]-MS_DF_filter$GFPavg)**2 + (MS_DF_filter$scale[1]-MS_DF_filter$scale)**2) )

## Add a column to include whether the protein's gene was DE in aDRG TMEM mutants
MS_DF_filter2 = MS_DF_filter
MS_DF_filter2$DEG_in_aDRG = ""
...
max(MS_DF_filter2$GFPavg)
max(MS_DF_filter2$TMEMavg)
max(MS_DF_filter2$scale)
## Create the proteins of interest variable for graphing colors
MS_DF_filter2$p.o.i. = ""
MS_DF_filter2$p.o.i.[c(6,7,9,10,11,13,15:20,22,24,26,27,29:31,36,38,40,42,43,47,48,51,56,63,69,71,74,76,77,80,83:91,96,97,98,100,102,105,107,108,109,113:117,123,124,126,127,132,134,136,138,141,143,144,145,149,152,154:156,158,159,166,167,172:177,183,184,186,190,193:196,198,203,204,209,212,219,222:227,230,231,236:240,244,247,259,297,308,312,313,316,340,343,347,349,353,372,380,403,439,471,493,501,521,527,531,533,535,539,541,542,546,547,550,556:561,563,566,568,570,573:575,579,590:593,599:601,607,612,614,620,643:648,657,673,674,691,764,775,777,789,793,797:799,804:806,809,811,815,817,819,822,824,833,839,841,843,846,849,857,861,868,874,876,879,885,914,918,932,967,972,980,985,991:996,998,1001,1008,1010,1011,1013,1014,1016:1018,1020,1029,1040,1047,1058,1078,1115,1116,1127,1140,1143,1145,1149,1154,1172,1173,1177:1180,1185,1205,1232,1235,1237,1239,1256,1258,1261,1263,1269,1272,1291,1292,1301,1322,1327,1336,1337,1339,1342,1345:1347,1355,1358,1367,1374,1377,1381,1476,1485,1490)] = "Erik's Hits"
MS_DF_filter2$p.o.i.[c(15,107,65,29,43,17,9,30,74,22,35,76,31,32,77,26,134,51,11,98,84,85,86,116,38,123,124,145,89,149,155,166,204,152,167,262,274,528,542,557,544,533,546,579,617,572,618,564,565,567,541,539,586,560,663,645,822,777,817,798,824,793,797,806,890,854,847,837,856,809,839,859,929,967,1013,971,1001,1003,991,995,996,1020,1052,1040,1008,1043,1087,993,1130,1115,1140,1145,1116,1149,1172,1177,1178,1232,1237,1239,1235,1327,1347,1348,1336,1345,1358,1346,1355,1339,1402,1408,1452,1459,1464,297,313,340,377)] = "Paul's Hits"
MS_DF_filter2$p.o.i.[which(match(MS_DF_filter2$Eriks_Likely_Candidates, MS_DF_filter2$Pauls_Likely_Candidates) > 1)] = "Overlapping Hits"
MS_DF_filter$p.o.i.[which(MS_DF_filter$p.o.i. != "Hits")] = "Background"
MS_DF_filter$p.o.i.[1] = "TMEM"
## If the "background" subset overwrites Paul's hits, restore them
MS_DF_filter2$p.o.i.[which(MS_DF_filter2$Pauls_Likely_Candidates != "")[(which(MS_DF_filter2$Pauls_Likely_Candidates != "") %notin% which(MS_DF_filter2$Eriks_Likely_Candidates != ""))]] = "Paul's Hits"



GG = ggplot(data = MS_DF_filter2) +
  geom_point(data = subset(MS_DF_filter2, p.o.i. == "Background"),
             aes(x = GFPavg, y = scale, color = p.o.i.), alpha = 0.1) +
  geom_point(data = subset(MS_DF_filter2, p.o.i. == "TMEM"),
             aes(x = GFPavg, y = scale, color = p.o.i.), alpha = 1.0, size = 2.5) +
  geom_point(data = subset(MS_DF_filter2, p.o.i. == "Paul's Hits"),
             aes(x = GFPavg, y = scale, color = p.o.i.), alpha = 0.3) +
  geom_point(data = subset(MS_DF_filter2, p.o.i. == "Erik's Hits"),
             aes(x = GFPavg, y = scale, color = p.o.i.), alpha = 0.3) +
  geom_point(data = subset(MS_DF_filter2, p.o.i. == "Overlapping Hits"),
             aes(x = GFPavg, y = scale, color = p.o.i.), alpha = 0.6) +
  coord_cartesian(xlim = c(0,0.05), ylim = c(0,1.2)) +
  labs(x = "Neg. Control Avg. Scaled Spectral Counts", y = "TMEM Avg. Scaled Spectral Counts (reg.)", title = "TMEM184B IP-MS Count Data", col = "Protein\nCategory") +
  #geom_hline(yintercept = TMEM_RATIO[1], color = "black")+
  scale_color_manual(values = c("gray48", "darkgoldenrod4", "navy", "forestgreen", "firebrick")) +
  theme_bw() +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.9), legend.position = "right")
...
GG = GG  + geom_text_repel(data = MS_DF_filter2, x = MS_DF_filter2$GFPavg, y = MS_DF_filter2$scale, color = "black", aes(label = labs), size = 1.8) 
...
MS_DF_filter$HumanGeneID[c(Human_BP_indeces_without_Ensembl_IDs, Human_BP_indeces_without_GO_IDs)]
  ## Concatenate the proteins' gene names into a list and rename the variable of the MS indeces with their gene names
Human_BPs_without_IDs = MS_DF_filter$HumanGeneID[c(Human_BP_indeces_without_GO_IDs)]
...
Find_GOs_of_Gene_X(GENE_ID = "SURF4")
...
Find_Genes_With_X_GO_Term(GO_Term = "recycling")
...
Find_Genes_Related_By_GO_Term(GO_Term1 = "axon", GO_Term2 = "synapse")
...
Find_Genes_Related_By_GO_Term(GO_Term1 = "generation of neurons", GO_Term2 = "response to axon injury")
  ...
  ##### GGPLOT2: For Erik's Hits as candidate list ######
library(PANTHER.db)
pthOrganisms(PANTHER.db) = "MOUSE"
library(enrichR)
library(org.Mm.eg.db)
...
DBs = DBs$libraryName[c(130,131,132,102,14,110)]
  ...
  ## Create the list!
MS_DF_filter$p.o.i. = ""
MS_DF_filter$p.o.i.[c(6,7,9,10,11,13,15:20,22,24,26,27,29:31,36,38,40,42,43,47,48,51,56,63,69,71,74,76,77,80,83:91,96,97,98,100,102,105,107,108,109,113:117,123,124,126,127,132,134,136,138,141,143,144,145,149,152,154:156,158,159,166,167,172:177,183,184,186,190,193:196,198,203,204,209,212,219,222:227,230,231,236:240,244,247,259,297,308,312,313,316,340,343,347,349,353,372,380,403,439,471,493,501,521,527,531,533,535,539,541,542,546,547,550,556:561,563,566,568,570,573:575,579,590:593,599:601,607,612,614,620,643:648,657,673,674,691,764,775,777,789,793,797:799,804:806,809,811,815,817,819,822,824,833,839,841,843,846,849,857,861,868,874,876,879,885,914,918,932,967,972,980,985,991:996,998,1001,1008,1010,1011,1013,1014,1016:1018,1020,1029,1040,1047,1058,1078,1115,1116,1127,1140,1143,1145,1149,1154,1172,1173,1177:1180,1185,1205,1232,1235,1237,1239,1256,1258,1261,1263,1269,1272,1291,1292,1301,1322,1327,1336,1337,1339,1342,1345:1347,1355,1358,1367,1374,1377,1381,1476,1485,1490)] = "Hits"
MS_DF_filter$p.o.i.[-c(6,7,9,10,11,13,15:20,22,24,26,27,29:31,36,38,40,42,43,47,48,51,56,63,69,71,74,76,77,80,83:91,96,97,98,100,102,105,107,108,109,113:117,123,124,126,127,132,134,136,138,141,143,144,145,149,152,154:156,158,159,166,167,172:177,183,184,186,190,193:196,198,203,204,209,212,219,222:227,230,231,236:240,244,247,259,297,308,312,313,316,340,343,347,349,353,372,380,403,439,471,493,501,521,527,531,533,535,539,541,542,546,547,550,556:561,563,566,568,570,573:575,579,590:593,599:601,607,612,614,620,643:648,657,673,674,691,764,775,777,789,793,797:799,804:806,809,811,815,817,819,822,824,833,839,841,843,846,849,857,861,868,874,876,879,885,914,918,932,967,972,980,985,991:996,998,1001,1008,1010,1011,1013,1014,1016:1018,1020,1029,1040,1047,1058,1078,1115,1116,1127,1140,1143,1145,1149,1154,1172,1173,1177:1180,1185,1205,1232,1235,1237,1239,1256,1258,1261,1263,1269,1272,1291,1292,1301,1322,1327,1336,1337,1339,1342,1345:1347,1355,1358,1367,1374,1377,1381,1476,1485,1490)] = "Background"
MS_DF_filter$p.o.i.[1] = "TMEM"


HitList = MS_DF_filter[which(MS_DF_filter$p.o.i. == "Hits"),]
HitList = HitList$HumanGeneID

  ## Run the data to acquire GO Terms
Human_Ensembl_BP_IDs = as.integer(
  mapIds(org.Hs.eg.db,
         keys = as.character(
           c(HitList[])
...
HitList[Human_BP_indeces_without_GO_IDs]
...
GO_INFO_df = GO_INFO_df[ , c(col_idx, (1:ncol(GO_INFO_df))[-c(col_idx)]) ]
  ...
  ##### Functional Analysis for Erik's Hit list #####
  ## Create a function that will return Enrichr-based functional analysis on a given gene set.
    ## First, create dataframes to store returned analyses as variables
GO_Processes = data.frame()
GO_Cell_Comps = data.frame()
...
Functional_Analysis = function(Genes){
  ...
  ## Run the data
Functional_Analysis(Genes = HitList)


  ## Create functions to find GO Terms associated with a protein of interest, intersecting between two proteins; or find proteins that are associated by a specific GO Term, or two proteins related by a GO Term
Find_GOs_of_Gene_X = function(GENE_ID) {
  return(str_split(
    as.vector(GO_INFO[ which(GO_INFO_df$GeneID == GENE_ID) ,   3]), pattern = ";", simplify = TRUE)[
      which(str_split(
...
Find_GOs_of_Gene_X(GENE_ID = "SURF4")
Find_GOs_of_Two_Genes(GENE_ID1 = "MTOR", GENE_ID2 = "JUN")
Find_GOs_of_Two_Genes(GENE_ID1 = "ULK1", GENE_ID2 = "JUN")
Find_GOs_of_Two_Genes(GENE_ID1 = "MTOR", GENE_ID2 = "ULK1")
Find_Genes_With_X_GO_Term(GO_Term = "immunity")


  ## Create new columns in the data frame to organize plotting aesthetics
MS_DF_filter$neurogenesis = MS_DF_filter$p.o.i.
MS_DF_filter$differentiation = MS_DF_filter$p.o.i.
MS_DF_filter$regeneration = MS_DF_filter$p.o.i.
MS_DF_filter$axon = MS_DF_filter$p.o.i.
MS_DF_filter$synapse = MS_DF_filter$p.o.i.
MS_DF_filter$autophagy = MS_DF_filter$p.o.i.
MS_DF_filter$tgf = MS_DF_filter$p.o.i.
MS_DF_filter$lysosome = MS_DF_filter$p.o.i.
MS_DF_filter$endosome = MS_DF_filter$p.o.i.

  ## Find the index of a given protein
which(MS_DF_filter$HumanGeneID == "LRSAM1")

  ## Populate the above columns with the indeces of the proteins associated with the GO Term and rename those indeces with the GO Term
MS_DF_filter$neurogenesis[c(7,17,27,36,38,42,51,90,102,113,143,247,539,550,592,612,645,648,673,793,815,857,918,985,991,1013,1140,1269)] = "Neurogenesis"
MS_DF_filter$differentiation[c(6,7,9,17,18,22,27,29,36,38,42,43,47,51,69,80,86,87,90,102,113,114,126,134,143,167,173,176,196,204,222,247,313,340,380,527,539,547,550,556,575,591,592,612,645,646,648,673,764,793,798,815,833,857,918,985,991,1013,1058,1140,1149,1154,1269,1381)] = "Differentiation"
MS_DF_filter$regeneration[c(36,38,42,43,69,592,648,673,691)] = "Regeneration"
MS_DF_filter$axon[c(7,17,36,38,42,51,102,113,143,176,247,592,612,648,673,857,995,1013,1058)] = "Axon"
MS_DF_filter$tgf[c(38,51,152,173,212,247,380,575)] = "TGF-\u03B2"
MS_DF_filter$synapse[c(7,15,17,18,102,109,113,143,550,574,673,811,857,991,1013,1029,1058,1337,1476)] = "Synapse"
MS_DF_filter$endosome[c(15,29,36,42,124,158,204,212,259,546,574,591,673,674,811,815,885,995,1011,1029,1115,1292,1355,1358,1377)] = "Endosome"
MS_DF_filter$lysosome[c(7,11,42,63,102,136,308,372,521,546,566,574,591,612,647,764,811,868,1292,1336)] = "Lysosome"
MS_DF_filter$autophagy[c(7,17,312,546,648,811,868,995,1011,1179,1292)] = "Autophagy"


##### Find proteins overlapping between (intersecting) two GO Terms #####

  ## Differentiation and Neurogenesis overlap
MS_DF_filter$HumanGeneID[which(MS_DF_filter$differentiation == "Differentiation")][which(!is.na(match(which(MS_DF_filter$neurogenesis == "Neurogenesis"), which(MS_DF_filter$differentiation == "Differentiation"))))]
  ## Differentiation and Regeneration overlap
MS_DF_filter$HumanGeneID[which(MS_DF_filter$differentiation == "Differentiation")][which(!is.na(match(which(MS_DF_filter$regeneration == "Regeneration"), which(MS_DF_filter$differentiation == "Differentiation"))))]
  ## Axon and synapse overlap
MS_DF_filter$HumanGeneID[which(MS_DF_filter$synapse == "Synapse")][which(!is.na(match(which(MS_DF_filter$synapse == "Synapse"), which(MS_DF_filter$axon == "Axon"))))]
  ## Axon and autophagy overlap
MS_DF_filter$HumanGeneID[which(MS_DF_filter$axon == "Axon")][which(!is.na(match(which(MS_DF_filter$autophagy == "Autophagy"), which(MS_DF_filter$axon == "Axon"))))]
  ## Lysosome and endosome overlap
MS_DF_filter$HumanGeneID[which(MS_DF_filter$lysosome == "Lysosome")][which(!is.na(match(which(MS_DF_filter$endosome == "Endosome"), which(MS_DF_filter$lysosome == "Lysosome"))))]
  ## Axon and regeneration overlap
MS_DF_filter$HumanGeneID[which(MS_DF_filter$axon == "Axon")][which(!is.na(match(which(MS_DF_filter$regeneration == "Regeneration"), which(MS_DF_filter$axon == "Axon"))))]


##### Neurogenesis and Differentiation #####
  ## Re-name strings in some of the columns to plot proteins in multiple GO Terms
    ## Overlap between neurogenesis and differentiation
which(MS_DF_filter$neurogenesis == "Neurogenesis")[which(!is.na(match(which(MS_DF_filter$neurogenesis == "Neurogenesis"), which(MS_DF_filter$differentiation == "Differentiation"))))]
    ## Protein indeces unique to differentiation (between neurogenesis and differentiation)
which(MS_DF_filter$differentiation == "Differentiation")[-c(which(!is.na(match(which(MS_DF_filter$differentiation == "Differentiation"), which(MS_DF_filter$neurogenesis == "Neurogenesis")))))]
    
    ## Re-name the protein indeces with the string of category term unique to differentiation in the neurogenesis column
MS_DF_filter$neurogenesis[which(MS_DF_filter$differentiation == "Differentiation")][-c(which(!is.na(match(which(MS_DF_filter$differentiation == "Differentiation"), which(MS_DF_filter$neurogenesis == "Neurogenesis")))))] = "Differentiation"

    ## Re-name the overlap in the neurogenesis column
MS_DF_filter$neurogenesis[which(MS_DF_filter$neurogenesis == "Neurogenesis")][which(!is.na(match(which(MS_DF_filter$neurogenesis == "Neurogenesis"), which(MS_DF_filter$differentiation == "Differentiation"))))] = "Neurogenesis\nand\nDifferentiation"

#MS_DF_filter$HumanGeneID[which(MS_DF_filter$differentiation == "Differentiation")][
#which(
#  which(MS_DF_filter$differentiation == "Differentiation") %notin% which(MS_DF_filter$neurogenesis == "Neurogenesis") == TRUE)]

##### Differentiation and Regeneration #####
  ## Re-name strings in some of the columns to plot proteins in multiple GO Terms
    ## Overlap between differentiation and regeneration
which(MS_DF_filter$differentiation == "Differentiation")[which(!is.na(match(which(MS_DF_filter$differentiation == "Differentiation"), which(MS_DF_filter$regeneration == "Regeneration"))))]
    ## Protein indeces unique to differentiation (between differentiation and regeneration)
which(MS_DF_filter$differentiation == "Differentiation")[-which(!is.na(match(which(MS_DF_filter$differentiation == "Differentiation"), which(MS_DF_filter$regeneration == "Regeneration"))))]
    ## Protein indeces unique to regeneration (between differentiation and regeneration)
which(MS_DF_filter$regeneration == "Regeneration")[-which(!is.na(match(which(MS_DF_filter$regeneration == "Regeneration"), which(MS_DF_filter$differentiation == "Differentiation"))))]

    ## Re-name the overlap in the differentiation column
MS_DF_filter$differentiation[which(MS_DF_filter$differentiation == "Differentiation")][which(!is.na(match(which(MS_DF_filter$differentiation == "Differentiation"), which(MS_DF_filter$regeneration == "Regeneration"))))] = "Regeneration\nand\nDifferentiation"

    ## Re-name the index unique to regeneration
MS_DF_filter$differentiation[c(582,636,657,672,691)] = "Regeneration"


##### Synapse and Axon #####
  ## Re-name strings in some of the columns to plot proteins in multiple GO Terms
    ## Overlap between synapse and axon
which(MS_DF_filter$synapse == "Synapse")[which(!is.na(match(which(MS_DF_filter$synapse == "Synapse"), which(MS_DF_filter$axon == "Axon"))))]
    ## Protein indeces unique to synapse (between synapse and axon)
which(MS_DF_filter$synapse == "Synapse")[-which(!is.na(match(which(MS_DF_filter$synapse == "Synapse"), which(MS_DF_filter$axon == "Axon"))))]
    ## Protein indeces unique to axon (between synapse and axon)
which(MS_DF_filter$axon == "Axon")[-which(!is.na(match(which(MS_DF_filter$axon == "Axon"), which(MS_DF_filter$synapse == "Synapse"))))]
  ## Re-name the indeces that overlap both terms in the synapse column
MS_DF_filter$synapse[ which(MS_DF_filter$synapse == "Synapse") ][which(!is.na(match(which(MS_DF_filter$synapse == "Synapse"), which(MS_DF_filter$axon == "Axon"))))] = "Synapse & Axon"
  ## Re-name the indeces that are unique to the axon in the synapse column
MS_DF_filter$synapse[c(36,38,42,51,176,247,592,612,648,995)] = "Axon" 


#MS_DF_filter$HumanGeneID[which(MS_DF_filter$differentiation == "Differentiation")][-c(which(!is.na(match(which(MS_DF_filter$differentiation == "Differentiation"), which(MS_DF_filter$neurogenesis == "Neurogenesis")))))]

#MS_DF_filter$HumanGeneID[which(MS_DF_filter$neurogenesis == "Neurogenesis")][c(which(!is.na(match(which(MS_DF_filter$neurogenesis == "Neurogenesis"), which(MS_DF_filter$differentiation == "Differentiation")))))]


MS_DF_filter$HumanGeneID[which(MS_DF_filter$neurogenesis == "Neurogenesis")]

## Create labels
MS_DF_filter$labs = ""
MS_DF_filter$labs[c(7,15,17,18,36,38,42,51,102,109,113,143,176)] = MS_DF_filter$HumanGeneID[c(7,15,17,18,36,38,42,51,102,109,113,143,176)]
MS_DF_filter$labs[c(7,15,17,18,36,38,42,51,102,109,113,143,176)]
which(MS_DF_filter$HumanGeneID == "SGTA")

  ## Find axes limits
max(MS_DF_filter$GFPavg[which(MS_DF_filter$synapse == "Hits")])
max(MS_DF_filter$TMEMavg)
max(MS_DF_filter$scale[which(MS_DF_filter$synapse == "Hits")])
MS_DF_filter = MS_DF_filter[-c(which(MS_DF_filter$scale == "NaN")),]


GG = ggplot(data = MS_DF_filter) +
  #geom_point(data = subset(MS_DF_filter, differentiation == "Background"),
  #           aes(x = GFPavg, y = scale, color = differentiation), alpha = 0.1) +
  geom_point(data = subset(MS_DF_filter, synapse == "Hits"),
             aes(x = GFPavg, y = scale, color = synapse), alpha = 0.3) +
  geom_point(data = subset(MS_DF_filter, synapse == "Synapse"),
             aes(x = GFPavg, y = scale, color = synapse), alpha = 0.5) +
  geom_point(data = subset(MS_DF_filter, synapse == "Synapse & Axon"),
             aes(x = GFPavg, y = scale, color = synapse), alpha = 0.5) +
  geom_point(data = subset(MS_DF_filter, synapse == "Axon"),
             aes(x = GFPavg, y = scale, color = synapse), alpha = 0.5) +
  geom_point(data = subset(MS_DF_filter, synapse == "TMEM"),
             aes(x = GFPavg, y = scale, color = synapse), alpha = 1) +
  coord_cartesian(xlim = c(0,0.11), ylim = c(0,1.72)) +
  labs(x = "Control IP Avg. Abundance", y = "TMEM IP Avg. Abundance", title = "TMEM184B IP-MS Count Data", col = "Protein\nCategory") +
  #geom_hline(yintercept = TMEM_RATIO[1], color = "black")+
  scale_color_manual(values = c("darkgoldenrod4", "navy", "red3", "forestgreen", "firebrick")) +
  theme_bw() +
  theme(panel.grid = element_blank(), plot.title = element_text(hjust = 0.9), legend.position = "right")
...
GG
  ...
  ## Add labels
GG = GG  + geom_text_repel(data = MS_DF_filter, x = MS_DF_filter$GFPavg, y = MS_DF_filter$scale, color = "black", aes(label = labs), size = 1.8) 
...
ggsave(filename = "IP-MS full raw", plot = last_plot(), device = "tiff", dpi = "print", units = "in", width = 5, height = 3.5, path = "M:/Erik/Data/Omics/MS")
