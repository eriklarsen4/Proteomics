...
MS_DF_filter6 = MS_DF_filter5 %>% filter(!grepl(MS_DF_filter5$`Identified Proteins (6608)` , pattern = "Keratin.+?$|Tubulin.+?$"))
  ## Coerce the MW column to be numeric
MS_DF_filter6$`Molecular Weight` = as.numeric(MS_DF_filter6$`Molecular Weight`)
...
BP_Candidate_List_Human = MS_DF_filter6$HumanGeneID[c(15,107,65,29,43,17,9,30,74,22,35,76,31,32,77,26,134,51,11,98,84,85,86,116,38,123,124,145,89,149,155,166,204,152,167,262,274,528,542,557,544,533,546,579,617,572,618,564,565,567,541,539,586,560,663,645,822,777,817,798,824,793,797,806,890,854,847,837,856,809,839,859,929,967,1013,971,1001,1003,991,995,996,1020,1052,1040,1008,1043,1087,993,1130,1115,1140,1145,1116,1149,1172,1177,1178,1232,1237,1239,1235,1327,1347,1348,1336,1345,1358,1346,1355,1339,1402,1408,1452,1459,1464,297,313,340,377)]


MS_DF_filter6$scale = ((MS_DF_filter6$TMEMavg*MS_DF_filter6$TMEM_RATIO)-(MS_DF_filter6$TMEMavg*sqrt((MS_DF_filter6$TMEM_RATIO - MS_DF_filter6$TMEM_RATIO[1])**2)))



##### GGPLOT #####
  ## Find the axis limits
max(MS_DF_filter6$GFPavg)
max(MS_DF_filter6$scale)
  ## Create the proteins of interest variable for graphing colors
MS_DF_filter6$p.o.i. = ""
MS_DF_filter6$p.o.i.[c(6,7,9,10,11,13,15:20,22,24,26,27,29:31,36,38,40,42,43,47,48,51,56,63,69,71,74,76,77,80,83:91,96,97,98,100,102,105,107,108,109,113:117,123,124,126,127,132,134,136,138,141,143,144,145,149,152,154:156,158,159,166,167,172:177,183,184,186,190,193:196,198,203,204,209,212,219,222:227,230,231,236:240,244,247,259,297,308,312,313,316,340,343,347,349,353,372,380,403,439,471,493,501,521,527,531,533,535,539,541,542,546,547,550,556:561,563,566,568,570,573:575,579,590:593,599:601,607,612,614,620,643:648,657,673,674,691,764,775,777,789,793,797:799,804:806,809,811,815,817,819,822,824,833,839,841,843,846,849,857,861,868,874,876,879,885,914,918,932,967,972,980,985,991:996,998,1001,1008,1010,1011,1013,1014,1016:1018,1020,1029,1040,1047,1058,1078,1115,1116,1127,1140,1143,1145,1149,1154,1172,1173,1177:1180,1185,1205,1232,1235,1237,1239,1256,1258,1261,1263,1269,1272,1291,1292,1301,1322,1327,1336,1337,1339,1342,1345:1347,1355,1358,1367,1374,1377,1381,1476,1485,1490)] = "Erik's Hits"
MS_DF_filter6$p.o.i.[c(15,107,65,29,43,17,9,30,74,22,35,76,31,32,77,26,134,51,11,98,84,85,86,116,38,123,124,145,89,149,155,166,204,152,167,262,274,528,542,557,544,533,546,579,617,572,618,564,565,567,541,539,586,560,663,645,822,777,817,798,824,793,797,806,890,854,847,837,856,809,839,859,929,967,1013,971,1001,1003,991,995,996,1020,1052,1040,1008,1043,1087,993,1130,1115,1140,1145,1116,1149,1172,1177,1178,1232,1237,1239,1235,1327,1347,1348,1336,1345,1358,1346,1355,1339,1402,1408,1452,1459,1464,297,313,340,377)] = "Paul's Hits"
MS_DF_filter6$p.o.i.[which(match(MS_DF_filter6$Eriks_Likely_Candidates, MS_DF_filter6$Pauls_Likely_Candidates) > 1)] = "Overlapping Hits"
MS_DF_filter6$p.o.i.[which(match(MS_DF_filter6$Eriks_Likely_Candidates, MS_DF_filter6$Pauls_Likely_Candidates) == 1)] = "Background"
MS_DF_filter6$p.o.i.[1] = "TMEM"
  ## If the "background" subset overwrites Paul's hits, restore them
MS_DF_filter6$p.o.i.[which(MS_DF_filter6$Pauls_Likely_Candidates != "")[(which(MS_DF_filter6$Pauls_Likely_Candidates != "") %notin% which(MS_DF_filter6$Eriks_Likely_Candidates != ""))]] = "Paul's Hits"

  ## Create labels
MS_DF_filter6$labs = ""
MS_DF_filter6$labs[c(35,42,77,86,132,7,11,77,36,80,85,98,100,144,145,38,31,74,126,152,154,297)] = MS_DF_filter6$HumanGeneID[c(35,42,77,86,132,7,11,77,36,80,85,98,100,144,145,38,31,74,126,152,154,297)]


GG = ggplot(data = MS_DF_filter6) +
  geom_point(data = subset(MS_DF_filter6, p.o.i. == "Background"),
             aes(x = GFPavg, y = scale, color = p.o.i.), alpha = 0.1) +
  geom_point(data = subset(MS_DF_filter6, p.o.i. == "TMEM"),
             aes(x = GFPavg, y = scale, color = p.o.i.), alpha = 1.0, size = 2.5) +
  geom_point(data = subset(MS_DF_filter6, p.o.i. == "Paul's Hits"),
             aes(x = GFPavg, y = scale, color = p.o.i.), alpha = 0.3) +
  geom_point(data = subset(MS_DF_filter6, p.o.i. == "Erik's Hits"),
             aes(x = GFPavg, y = scale, color = p.o.i.), alpha = 0.3) +
  geom_point(data = subset(MS_DF_filter6, p.o.i. == "Overlapping Hits"),
             aes(x = GFPavg, y = scale, color = p.o.i.), alpha = 0.6) +
  coord_cartesian(xlim = c(0,0.25), ylim = c(0,3)) +
...
GG = GG  + geom_text_repel(data = MS_DF_filter6, x = MS_DF_filter6$GFPavg, y = MS_DF_filter6$scale, color = "black", aes(label = labs), size = 1.8) 
